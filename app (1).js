...
//  crea el SFM al contratar, y deja constancia en Colaboradores/{uid},
...
//  mismos documentos que usa el software del SFM.
// ══════════════════════════════════════════════════════════
...
async function loadSession(user) {
  // Colaboradores/{uid} es lo que el SFM creó al contratar.
  const link = await db.collection('Colaboradores').doc(user.uid).get();
  if (!link.exists) {
    throw new Error('Esta cuenta no está vinculada a ningún colaborador. Pídele a tu SFM que revise tu alta.');
  }
  const empId = link.data().employeeId;
  const doc   = await db.collection('Employees').doc(empId).get();
  if (!doc.exists) throw new Error('No encontramos tu ficha. Habla con tu SFM.');

  const e = doc.data();
  if (e.status !== 'active') throw new Error('Tu cuenta está inactiva. Habla con tu SFM.');
...
/* Cuando el SFM marca un día de descanso lo guarda como { libre:true },
...
async function ensureDm(otherPid, names) {
  ...
  // y si el SFM había creado el chat con los participantes en otro
  // orden, la regla veía un "cambio de miembros", lo negaba, y el chat con
  // el SFM no abría. Leer también es más rápido que escribir.
...
// ── Mensaje del día que dejó el SFM ──
